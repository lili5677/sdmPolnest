...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $gaji_min = isset($gaji[0]) ? (int)trim($gaji[0]) : 0;
        $gaji_max = isset($gaji[1]) ? (int)trim($gaji[1]) : 0;

        // Validasi gaji
        if ($gaji_min <= 0 || $gaji_max <= 0) {
            throw new Exception('Format rentang gaji tidak valid. Contoh: 2000000 - 3500000');
        }
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        input[type="text"], input[type="number"], input[type="date"], textarea { padding: 12px 16px; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 14px; font-family: 'Poppins', sans-serif; transition: border-color 0.2s; width: 100%; }
        ...
        .btn { padding: 12px 24px; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; border: none; transition: all 0.2s; text-decoration: none; display: inline-flex; align-items: center; gap: 8px; }
...
<body>
                    ...
                    <form method="POST">
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input type="text" id="posisi" name="posisi" placeholder="Contoh: Dosen Teknik Informatika" required value="<?= isset($_POST['posisi']) ? htmlspecialchars($_POST['posisi']) : '' ?>">
                            </div>
                            <div class="form-group">
                                <label for="formasi">Formasi <span class="required">*</span></label>
                                <input type="number" id="formasi" name="formasi" placeholder="Jumlah formasi" min="1" required value="<?= isset($_POST['formasi']) ? htmlspecialchars($_POST['formasi']) : '' ?>">
                                <span class="input-helper">Jumlah orang yang dibutuhkan</span>
                            </div>
                            <div class="form-group">
                                <label for="gaji_range">Rentang Gaji <span class="required">*</span></label>
                                <input type="text" id="gaji_range" name="gaji_range" placeholder="Contoh: 8000000 - 12000000" required value="<?= isset($_POST['gaji_range']) ? htmlspecialchars($_POST['gaji_range']) : '' ?>">
                                <span class="input-helper">Format: angka - angka (tanpa titik/koma)</span>
                            </div>
                            <div class="form-group">
                                <label for="deadline_lamaran">Deadline Lamaran <span class="required">*</span></label>
                                <input type="date" id="deadline_lamaran" name="deadline_lamaran" required min="<?= date('Y-m-d') ?>" value="<?= isset($_POST['deadline_lamaran']) ? htmlspecialchars($_POST['deadline_lamaran']) : '' ?>">
    ...
    <script>
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            const gajiRange = document.getElementById('gaji_range').value;
